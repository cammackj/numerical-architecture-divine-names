...
42 & mediation and the 42-letter tradition & Eloah: $42_{10}$; outer Ehyeh pair: $21+21=42$ & exact decimal total and subtotal \\
...
A second contact appears in the declaration of Exodus 3:14.\cite{exodus314} Ehyeh has value 21, so its repeated outer occurrences in Ehyeh Asher Ehyeh contribute 42 before the central word is added:

\[
\text{Ehyeh}+\text{Ehyeh}=21+21=42.
\]

These are exact calculations, but they are not the same kind of calculation. Eloah has the complete value 42; the Ehyeh pair contributes a subtotal of 42 within the complete value 543. The subtotal is textually motivated by the repeated outer frame, but its significance was recognized after inspection and is not assigned a prospective probability. The 42-Letter Name, attested as a guarded tradition in \emph{Kiddushin} 71a, is a separate formula object rather than an ordinary lexical name.\cite{kiddushin42} Its designation should not be treated as though the English title itself had gematria 42 or as though it were derived from the Exodus subtotal.

Within the interpretive model, 42 can function as a mediator because it joins an exact divine-name total, a repeated outer frame in a complete formula, and the 42-letter tradition. That role is a synthesis of three distinct contacts, not an identity among them or a transition law derived from arithmetic.
...
The corpus gives the ordering several nontrivial anchors: YHWH supplies $22_{12}$, Eloah supplies 42 directly, the outer Ehyeh pair supplies the subtotal 42 compositionally, the 72 tradition supplies the canonical reference, and Elohim supplies $72_{12}$. The corrected AB label states decimal 72 conventionally. What has not been demonstrated is a mathematical operation that transforms 22 into 42 and then 42 into 72, or a statistical result showing that these three references govern the whole corpus.
